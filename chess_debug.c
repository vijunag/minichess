...
int GameScore(Board p1, Board p2, int depth)
{
  if (!p2.qu) {
    ...
    return GetBoardScore(p2) - GetBoardScore(p1) - depth;
  }
}
...
#if 0
#define FOREACHMOVES(p,_moves,_wb,_bb,max)                            \
  if (_moves&&!done) {                                                \
    for(uint16_t mask=0x8000;mask;mask>>=1) {                         \
      if (_moves&mask) {                                              \
        if (max) {                                                    \
          Board nwb=_wb;                                              \
          nwb.set(p, mask);                                           \
          score=MAX(score, minimax(_bb,nwb,alpha,beta,depth-1, 0));   \
          alpha=MAX(alpha,score);                                     \
          printf("Value of alpha at depth:%d=%d\n", depth, alpha);    \
          if (beta<=alpha)  {                                         \
            done=1;                                                   \
            printf("Maxprun at depth:%d.Best score=%d\n",depth,alpha);\
            break;                                                    \
          }                                                           \
        } else {                                                      \
          Board nbb=_bb;                                              \
          nbb.set(p, mask);                                           \
          score=MIN(score,minimax(nbb,_wb,alpha,beta,depth-1, 1));    \
          beta=MIN(beta,score);                                       \
          printf("Value of Beta at depth:%d=%d\n", depth, beta);      \
          if (beta<=alpha) {                                          \
            done=1;                                                   \
            printf("Minprun at depth:%d.Best score=%d\n",depth,alpha);\
            break;                                                    \
          }                                                           \
        }                                                             \
      }                                                               \
...
#define MINIMAX(_wb,_bb,max)                 \
 FOREACHMOVES(QUEEN, qmoves,_wb, _bb,max);   \
 FOREACHMOVES(KNIGHT1,kmoves,_wb, _bb,max);  \
 FOREACHMOVES(KNIGHT2,kmoves2,_wb, _bb,max); \
 FOREACHMOVES(ROOK1,  rmoves,_wb, _bb,max);  \
 FOREACHMOVES(ROOK2,  rmoves2,_wb, _bb,max); \
 FOREACHMOVES(BISHOP1,bmoves,_wb, _bb,max);  \
 FOREACHMOVES(BISHOP2,bmoves2,_wb, _bb,max);
#endif
int g_depth;
int minimax(Board b, Board w, int alpha, int beta, int depth, int maximize)
{
  int score = GameScore(b, w, (g_depth-depth));

#if DEBUG
  PrintGameBoard(b, w, maximize);
  printf("Max:%d, Ply: %d, Score: %d, Alpha: %d, Beta: %d\n", maximize, \
      ...
      if (mv&&!done) {
        for(uint16_t mask=0x8000;mask;mask>>=1) {
          if (mask&mv) {
            ...
            val=minimax(nbb,nwb,alpha,beta,depth-1,0);
            if (score<val && (depth==g_depth)) {
              fwb = nwb;
            }
#ifdef LOGMOVES
            if (depth == g_depth) {
              printf("Score for piece=%c is %d, curmax=%d\n",
                  GetPieceName(p), val, MAX(score,val));
              PrintGameBoard(nbb, nwb, maximize);
            }
#endif /*LOGMOVES*/
#ifdef DEBUG
            printf("Max score at depth:%d is %d\n", depth, val);
#endif /*DEBUG*/
            score=MAX(score,val);
            alpha=MAX(alpha,score);
            if (beta<=alpha) {
      ...
      if (mv&&!done) {
        for(uint16_t mask=0x8000;mask;mask>>=1) {
          if (mask&mv) {
            ...
            Board nwb=w;
            nbb.set(p, mask);
            nwb.clear(nbb.getbits()); //white captures
            val=minimax(nbb,nwb,alpha,beta,depth-1,1);
#ifdef LOGMOVES
            if (depth == g_depth) {
              printf("Score for piece=%c is %d, curmin=%d\n",
                  GetPieceName(p), val, MIN(score,val));
              PrintGameBoard(nbb, nwb, maximize);
            }
#endif /*LOGMOVES*/
#ifdef DEBUG
            printf("Min score at depth:%d is %d\n", depth, val);
#endif/*DEBUG*/
            score=MIN(score,val);
            if (score >= val && (g_depth==depth)) {
              fbb = nbb;
            }
            beta=MIN(beta,score);
            if (beta<=alpha) {
...
int main(int argc, char **argv) {
  ...
  while (g--) {
    ...
    fbb = black;
    fwb = white;
    score = minimax(black, white, LOSESCORE, WINSCORE, m, 1);
    if (WINSCORE==score) {
      printf("YES\n");
    } else {
      printf("NO\n");
